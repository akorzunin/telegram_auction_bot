...
from telegram import InlineKeyboardButton, InlineKeyboardMarkup, InputMediaPhoto, ReplyKeyboardMarkup, Update, ReplyKeyboardRemove
...
def show_data(update: Update, context: CallbackContext) -> None:
    ...
    update.message.reply_text(
        f"Current post data: {facts_to_str(context.user_data)}"
    )
...
def send_post_to_channel(update: Update, context: CallbackContext, chat_id: int, item_id):
    '''require 
    ...
    def fix_md(text: str) -> str:
        return text \
            ...
            .replace('-', '\-')
    buttons = [
        [InlineKeyboardButton("Get price", callback_data=f"get_price_callback id={item_id}")], 
...
post_upload_conversation = ConversationHandler(
        ...
        states={
            TEXT_CHOISE: [
                MessageHandler(
                    Filters.regex('^(Add title|Add description|Add price)$'), 
                    regular_choice
                ),
                MessageHandler(
                    Filters.regex('^(Add photo)$'), 
                    photo_choice
                ),
                MessageHandler(
                    Filters.regex('^(Preview)$'), 
                    show_data
                ),
                MessageHandler(
